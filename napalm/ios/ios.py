...
from napalm import ReplaceConfigException, MergeConfigException, \
            ConnectionClosedException, CommandErrorException

from napalm import py23_compat
from napalm import constants as C
from napalm import helpers
...
class IOSDriver(NetworkDriver):
    ...
    def get_interfaces(self):
        ...
        for line in output.splitlines():
            ...
            if re.search(mac_addr_regex, line):
                ...
                mac_address = helpers.mac(mac_addr_match.groups()[0])
    ...
    def get_bgp_neighbors(self):
        ...
        router_id = helpers.ip(router_id, version=4)

        # add parsed data to output dict
        bgp_neighbor_data['global']['router_id'] = router_id
        bgp_neighbor_data['global']['peers'] = {}
        for entry in summary_data:
            remote_addr = helpers.ip(entry['remote_addr'])
            afi = entry['afi'].lower()
            ...
            for neighbor in neighbor_data:
                if (neighbor['afi'].lower() == afi and
                        helpers.ip(neighbor['remote_addr']) == remote_addr):
            ...
            remote_id = helpers.ip(neighbor_entry['remote_id'], version=4)

            if remote_addr not in bgp_neighbor_data['global']['peers']:
                bgp_neighbor_data['global']['peers'][remote_addr] = {
                    'local_as': helpers.as_number(entry['local_as']),
                    'remote_as': helpers.as_number(entry['remote_as']),
                    'remote_id': remote_id,
                    'is_up': is_up,
                ...
                assert existing['local_as'] == helpers.as_number(entry['local_as'])
                assert existing['remote_as'] == helpers.as_number(entry['remote_as'])
